...
class LidarSubscriber:
    def __init__(self):
        rospy.init_node('distance_front', anonymous=True)
    ...
    def lidar_callback(self, data):
        ...
        if object_detected:
            self.distance_pub.publish(closest_distance - 2.5)
            rospy.loginfo(f"Distance: {closest_distance}")
        else:
            self.distance_pub.publish(self.no_object_distance)
