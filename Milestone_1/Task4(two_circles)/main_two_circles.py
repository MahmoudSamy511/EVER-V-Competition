...
from two_circles import two_circles
...
class OdomImuLogger:
    ...
    def imu_callback(self, msg):
        ...
        time = datetime.fromtimestamp(rospy.Time.now().to_sec()).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
        imu_values = [time, msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z, msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.angular_velocity.x, msg.angular_velocity.y, msg.angular_velocity.z]
    ...
    def save_to_csv(self):
        ...
        with open("data/imu_data.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["Timestamp", "Linear_Acceleration_X", "Linear_Acceleration_Y", "Linear_Acceleration_Z", "Orientation_X", "Orientation_Y", "Orientation_Z", "Angular_Velocity_X", "Angular_Velocity_Y", "Angular_Velocity_Z"])
