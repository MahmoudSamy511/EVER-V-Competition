...
class ImageSubscriber:
    def __init__(self):
        ...
        self.intersection_pub = rospy.Publisher('/lane_intersection_point', Point, queue_size=10)
    ...
    def detect_lanes(self, cv_image):
        ...
        if lines is not None:
            ...
            if self.left_line and self.right_line:
                left_slope = (self.left_line[1][1] - self.left_line[0][1]) / (self.left_line[1][0] - self.left_line[0][0])
                right_slope = (self.right_line[1][1] - self.right_line[0][1]) / (
                            self.right_line[1][0] - self.right_line[0][0])

                x_intersect = int(
                    ((self.left_line[0][1] - (left_slope * self.left_line[0][0])) - (self.right_line[0][1] - (
                            right_slope * self.right_line[0][0]))) / (
                                left_slope - right_slope))
                y_intersect = int(
                    ((left_slope * x_intersect) + (self.left_line[0][1] - (left_slope * self.left_line[0][0]))))

                intersection_point = Point()
                intersection_point.x = x_intersect
                intersection_point.y = y_intersect

                self.intersection_pub.publish(intersection_point)
